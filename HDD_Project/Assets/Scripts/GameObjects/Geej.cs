...
using TMPro;
using System;
public class Geej : MonoBehaviour
{
    ...
    private SpriteRenderer geejRender;
    private CircleCollider2D geejCollider;
    private Player player;

    private float lastXPos = 0;
    private float currX;
    
    public Animator geej_anim;
    ...
    void Start()
    {
        geejRender = GetComponent<SpriteRenderer>();
        geejCollider = GetComponent<CircleCollider2D>();
        ...
        if (other.CompareTag("HitBox"))
        {
            if (!isTakingDamage && !isDead)
            {
                ...
                if (currHealth <= 0)
                {
                    ...
                    geejCollider.enabled = false;
                    geejRender.enabled = false;
                    isDead = true;
    ...
    private void PlayerIsInRange()
    {
        ...
        else if (Vector3.Distance(target.position, transform.position) < 1 && !isIdle)
        {
            isIdle = true;
    ...
    void Animate()
    {
        geej_anim.SetBool("isLeft", isLeft);
        geej_anim.SetBool("isPuking", isPuking);
        geej_anim.SetBool("isMoving", isMoving);
    }
    ...
    private IEnumerator PukingAnim()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            if (i == 1)
            {
                isPuking = false;
                //TODO: spawn puke
                Vector3 spawnLoc = new Vector3(transform.position.x, transform.position.y - .5f, 0);
    ...
    private void SetStats()
    {
        ...
        if (lvl < 5)
        {
            baseHealth = 40 + (int)(lvl * 5);
            baseAttack = 15 + (int)(lvl * 1.5);
            baseDefense = 32 + (int)(lvl * 2);
            baseSpeed = 3.5f + (lvl * .02f);
            currHealth = baseHealth;
        }
        else
        {
            baseHealth = 40 + (int)(lvl * 10);
            baseAttack = 15 + (int)(lvl * 3);
            baseDefense = 32 + (int)(lvl * 4);
            baseSpeed = 3.5f + (lvl * .04f);
            currHealth = baseHealth;
        }
    }
}
